...
class FilterController(QObject):
    ...
    def get_filtered_tuple_pairs(self, label):
        ...
        data = data.iloc[0 * ApplicationContext.tuple_pair_count_per_page: 0 * ApplicationContext.tuple_pair_count_per_page + ApplicationContext.tuple_pair_count_per_page]
        # todo 4/7/17 get attributes from data
        self.main_page.setHtml(
            Renderer.render_main_page(tuple_pairs=data, attributes=ApplicationContext.current_attributes, current_page=0,
                                      ...
                                      total_count=ApplicationContext.COMPLETE_DATA_FRAME.shape[0],
                                      match_count=ApplicationContext.COMPLETE_DATA_FRAME[ApplicationContext.COMPLETE_DATA_FRAME.label == ApplicationContext.MATCH].shape[0],
                                      not_match_count=ApplicationContext.COMPLETE_DATA_FRAME[ApplicationContext.COMPLETE_DATA_FRAME.label == ApplicationContext.NON_MATCH].shape[0],
                                      not_sure_count=ApplicationContext.COMPLETE_DATA_FRAME[ApplicationContext.COMPLETE_DATA_FRAME.label == ApplicationContext.NOT_SURE].shape[0],
                                      unlabeled_count=ApplicationContext.COMPLETE_DATA_FRAME[ApplicationContext.COMPLETE_DATA_FRAME.label == ApplicationContext.NOT_LABELED].shape[0],
    ...
    def filter_attribute(self, attributes):
        ...
        html = Renderer.render_main_page(tuple_pairs=pagination_contoller.get_page(0),
                                         ...
                                         count_per_page=ApplicationContext.tuple_pair_count_per_page,
                                         number_of_pages=ceil(ApplicationContext.current_data_frame.shape[0] / ApplicationContext.tuple_pair_count_per_page),
                                         total_count=stats_controller.count_tuple_pairs(ApplicationContext.current_data_frame),
